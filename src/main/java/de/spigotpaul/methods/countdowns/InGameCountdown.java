	...
	public void startCountdown() {
		...
		for (int i = 0; i != 1000; i++) {
			for(Player all : Bukkit.getOnlinePlayers()) {
				JumpLeague.getInstance().getScoreboardManager().updateInGameTime(all);
			}
			...
			if (inGameCountdown == 10 || inGameCountdown <= 5 && inGameCountdown >= 2) {
				for (Player all : Bukkit.getOnlinePlayers()) {
					all.playSound(all.getLocation(), Sound.NOTE_BASS_DRUM, 1, 1);
				}

				Bukkit.broadcastMessage(JumpLeague.getInstance().getPrefix() + "§7Das §eSpiel §7endet in §8(§e" + inGameCountdown + "§8) §7Sekunden§8!");
			} else if (inGameCountdown == 1) {
				for (Player all : Bukkit.getOnlinePlayers()) {
					all.playSound(all.getLocation(), Sound.NOTE_BASS_DRUM, 1, 1);
				}
				...
				for(Player all : Bukkit.getOnlinePlayers()) {
					all.teleport(JumpLeague.getInstance().getLocationManager().getInGameLocation(all, "deathmatch", teleportKey));
					
					JumpLeague.getInstance().getScoreboardManager().setDeathMatchScoreboard(all);
